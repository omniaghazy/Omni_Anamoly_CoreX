...
def create_anomaly_test_set(normal_test_data, all_columns, numeric_features, num_anomalies=10):
    ...
    if not allowed_indices:
        print("⚠️ Warning: No valid sensors found!")
        return test_data, final_labels
    ...
    print(f"✅ Success! Injected {np.sum(final_labels == 1)} points (Spikes/Drifts/Stucks).")
    return test_data, final_labels
...
if __name__ == "__main__":
    ...
    if os.path.exists(your_file):
        ...
        print(f"--- [Testing] Data Loaded. Shape: {numeric_df.shape} ---")
        ...
        print(f"🔥 Success! Total anomaly points (1s) found: {total_anomalies}")
        
        if total_anomalies > 0:
            print("🚀 Great! The injection logic is working and saving labels.")
        else:
            print("⚠️ Still Zero? Check if start_idx and buffer logic are skipping segments.")
            
    else:
        print(f"❌ Error: File '{your_file}' not found. Check your folders!")