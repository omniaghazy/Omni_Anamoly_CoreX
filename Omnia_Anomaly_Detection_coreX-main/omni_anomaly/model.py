...
class OmniAnomaly(VarScopeObject):
    ...
    def get_training_loss(self, x, n_z=None):
        with tf.name_scope('training_loss'):
            # 1. بناء الـ VAE Chain (دي خلاص صلحناها في vae.py وشغالة تمام)
            chain = self.vae.chain(x, n_z=n_z, posterior_flow=self._posterior_flow)
            
            # 2. 🔥 بدل العك اليدوي.. هنستخدم الـ vi الجاهزة جوه الـ chain
            # الـ chain.vi.training.sgvb() بتحسب الـ Loss وبتاخد بالها من الأبعاد لوحدها
            sgvb_loss = tf.reduce_mean(chain.vi.training.sgvb())
            
            # 3. إضافة الـ L2 Regularization (زي ما هي عشان الـ Overfitting)
