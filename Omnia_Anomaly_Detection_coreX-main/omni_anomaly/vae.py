...
class VAE(VarScopeObject):
    ...
    def variational(self, x, z=None, n_z=None, posterior_flow=None):
        ...
        with tf.name_scope('z'):
            # بناخد الـ params ونقسمهم
            input_q = z_params['input_q']
            z_mean, z_logstd = tf.split(input_q, 2, axis=-1)
            
            # هنا بنبني التوزيع بالـ shape اللي طالع فعلاً 
            q_z_dist = Normal(mean=z_mean, std=tf.exp(z_logstd))
            
            z = net.add('z', q_z_dist, n_samples=n_z, 
    ...
    def model(self, z=None, x=None, n_z=None, n_x=None):
        ...
        with tf.name_scope('z'):
            # لو الـ z جاية من الـ variational (في الـ training)
            # هنخلي الـ prior ياخد شكلها بالظبط عشان الـ broadcasting ينجح
            if z is not None:
                z_mean = tf.zeros_like(z)
                z_std = tf.ones_like(z)
            else:
                z_dim_val = getattr(self, 'z_dim', 3)
                z_mean = tf.zeros([z_dim_val])
                z_std = tf.ones([z_dim_val])
                
            p_z_dist = Normal(mean=z_mean, std=z_std)
            z = net.add('z', p_z_dist, n_samples=n_z,
                        group_ndims=self.z_group_ndims,
                        is_reparameterized=self.is_reparameterized)
        ...
        with tf.variable_scope('p_x_given_z'):
            # بنسحب المتوسط والانحراف المعياري اللي الديكودر طلعهم
            p_x_dist = Normal(mean=x_params['mean'], std=x_params['std'])
            
        with tf.name_scope('x'):
            x = net.add('x', p_x_dist, n_samples=n_x, group_ndims=self.x_group_ndims)
    ...
    def get_training_loss(self, x, n_z=None):
            with tf.name_scope('VAE.get_training_loss'):
                ...
                # بنجهز الـ chain والـ z زي ما هم
                chain = self.chain(x, n_z)
                z = chain.variational['z']

                # --- التعديل بيبدأ من هنا (إلغاء الاختيار التلقائي) ---
                # بدل الـ if/else المعقدة، هنثبت الـ Solver الأقوى لمشروعك
                if z.is_reparameterized:
                    # إجبار استخدام IWAE لضمان أعلى دقة في الـ Reconstruction للروبوت
                    solver = chain.vi.training.iwae 
                else:
                    # كبديل لو الـ z مش reparameterized بنستخدم VIMCO
                    solver = chain.vi.training.vimco
                # --- نهاية التعديل ---

                # بنحسب الـ loss النهائي
