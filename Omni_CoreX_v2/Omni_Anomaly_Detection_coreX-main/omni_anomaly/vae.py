...
class VAE(VarScopeObject):
    ...
    def variational(self, x, z=None, n_z=None, posterior_flow=None):
        ...
        with tf.name_scope('z'):
            q_z_dist = self.q_z_given_x(**z_params)

            # Apply posterior flow if specified via FlowDistribution
            if posterior_flow is not None:
                from tfsnippet.distributions import FlowDistribution
                q_z_dist = FlowDistribution(q_z_dist, posterior_flow)
    ...
    def model(self, z=None, x=None, n_z=None, n_x=None):
        ...
        with tf.name_scope('x'):
            x = net.add('x', p_x_dist, n_samples=n_x,
