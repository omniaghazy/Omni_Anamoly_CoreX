...
class TfpDistribution(Distribution):
    ...
    def log_prob(self, given, group_ndims=0, name=None):
        with tf.name_scope(name=name, default_name='log_prob'):
            # تحسين 4: الـ forward_filter هنا لازم نتأكد إنها بتتعامل مع الـ Dynamic Window 
            # اللي عملناه في الـ RNN. 
            # الكود الأصلي كان بيرجع 7 قيم، إحنا يهمنا أول قيمة (Log Probability)
...
def wrap_params_net(inputs, h_for_dist, mean_layer, std_layer):
    ...
    # 3. حساب المتوسط (الـ Mean) عادي جداً
    z_mean = mean_layer(h)
